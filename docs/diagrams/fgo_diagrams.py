...
for i in range(4):
    ...
    if i < 3:
        ...
        pgm_between.add_node(f"dvlb{i}", f"$f^\\mathcal{{D}}_{{{i}{i+1}}}$", mid_x, 3, fixed=True, plot_params=style_factor_dvl, offset=[0, 3])
...
pgm_between.figure.savefig(OUTPUT_DIR / "fgo_dvl_between.png", bbox_inches="tight", dpi=300)
